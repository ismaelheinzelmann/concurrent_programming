...
void *add_counter(void * loops)
{
    int * loops_int = (int *)loops;
    for (int i = 0; i < *loops_int; i++)
    {
        contador_global++;
    }
    pthread_exit(NULL);
...
int main(int argc, char *argv[])
{
    ...
    {
        pthread_create(&threads[i], NULL, add_counter, (void *)&n_loops);
    }
...
// Condição de corrida gera a diferença entre esperado e contador, que com o aumento do número de threads, gera uma diferença cada vez maior.